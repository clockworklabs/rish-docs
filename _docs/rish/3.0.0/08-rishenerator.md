...
If your struct defines a `[Default]` property or has any fields with `[DefaultValue]` or `[DefaultCode]` attributes, Rishenerator injects logic to respect those values when arguments are omitted.
...
For Props using the `[Expand]` attribute in `VisualAttributes` fields, the generator "flattens" them into `name`, `className`, and `style` arguments.
...
<div class="callout-block callout-block-danger">
    <div class="content">
        <h4 class="callout-title"><i class="fa-solid fa-triangle-exclamation"></i>Reserved Keywords</h4>
        <ul>
